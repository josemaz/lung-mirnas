...
##########################
## LumA
ruta <- paste(casedir, "LumA/RNAseq", sep="/")
cases <- read.table(paste(ruta, "db.csv", sep="/"),header=F,sep=",")
cases$V2 <- gsub(".{3}$", "", cases$V2)
cases$V2 <- paste(ruta,cases$V2, sep="/")
cat('LumA directory: ', ruta, '\n')
LumAData<-bplapply(cases$V2, read.delim, sep="\t", header=F, col.names=c("EnsemblID", "raw_counts"))

##Check if all samples have the same size
size<-unique(do.call(rbind,lapply(LumAData, dim)))
stopifnot(nrow(size)==1)
cat('LumA samples have the same size \n')

##Check if the genes match positions
genes<-do.call(cbind,lapply(LumAData, function(x)as.character(x[,1])))
genes<-t(unique(t(genes)))
stopifnot(dim(genes)==c(size[1,1], 1))
cat('Genes in LumA samples match positions \n')

##Let's keep only the raw counts
LumA<-do.call(cbind, lapply(LumAData, function(x)x[,"raw_counts"]))
targets<-data.frame(File=cases$V2, ID=paste("tLA", 1:length(cases$V2), sep=""), CASEID=cases$V1)
colnames(LumA)<-targets$ID
...
##Save clean data
LumA<-list(Counts=LumA, Annot=genes, targets=targets)
save(LumA, file=paste(RDATA, "LumARaw.RData", sep="/"), compress="xz")
cat('LumARaw.RData saved \n')


##########################
## LumB
ruta <- paste(casedir, "LumB/RNAseq", sep="/")
cases <- read.table(paste(ruta, "db.csv", sep="/"),header=F,sep=",")
cases$V2 <- gsub(".{3}$", "", cases$V2)
cases$V2 <- paste(ruta,cases$V2, sep="/")
cat('LumB directory: ', ruta, '\n')
LumBData<-bplapply(cases$V2, read.delim, sep="\t", header=F, col.names=c("EnsemblID", "raw_counts"))

##Check if all samples have the same size
size<-unique(do.call(rbind,lapply(LumBData, dim)))
stopifnot(nrow(size)==1)
cat('LumB samples have the same size \n')

##Check if the genes match positions
genes<-do.call(cbind,lapply(LumBData, function(x)as.character(x[,1])))
genes<-t(unique(t(genes)))
stopifnot(dim(genes)==c(size[1,1], 1))
cat('Genes in LumB samples match positions \n')

##Let's keep only the raw counts
LumB<-do.call(cbind, lapply(LumBData, function(x)x[,"raw_counts"]))
targets<-data.frame(File=cases$V2, ID=paste("tLB", 1:length(cases$V2), sep=""), CASEID=cases$V1)
colnames(LumB)<-targets$ID

##Let's change the annotation ensembleid.version
genes<-do.call(rbind,sapply(genes[,1], strsplit, split=".", fixed=TRUE))
colnames(genes)<-c("EnsemblID", "version")

##Save clean data
LumB<-list(Counts=LumB, Annot=genes, targets=targets)
save(LumB, file=paste(RDATA, "LumBRaw.RData", sep="/"), compress="xz")
cat('LumBRaw.RData saved \n')


##########################
## Her2
ruta <- paste(casedir, "Her2/RNAseq", sep="/")
cases <- read.table(paste(ruta, "db.csv", sep="/"),header=F,sep=",")
cases$V2 <- gsub(".{3}$", "", cases$V2)
cases$V2 <- paste(ruta,cases$V2, sep="/")
cat('Her2 directory: ', ruta, '\n')
Her2Data<-bplapply(cases$V2, read.delim, sep="\t", header=F, col.names=c("EnsemblID", "raw_counts"))

##Check if all samples have the same size
size<-unique(do.call(rbind,lapply(Her2Data, dim)))
stopifnot(nrow(size)==1)
cat('Her2 samples have the same size \n')

##Check if the genes match positions
genes<-do.call(cbind,lapply(Her2Data, function(x)as.character(x[,1])))
genes<-t(unique(t(genes)))
stopifnot(dim(genes)==c(size[1,1], 1))
cat('Genes in Her2 samples match positions \n')

##Let's keep only the raw counts
Her2<-do.call(cbind, lapply(Her2Data, function(x)x[,"raw_counts"]))
targets<-data.frame(File=cases$V2, ID=paste("tH2", 1:length(cases$V2), sep=""), CASEID=cases$V1)
colnames(Her2)<-targets$ID
...
##Save clean data
Her2<-list(Counts=Her2, Annot=genes, targets=targets)
save(Her2, file=paste(RDATA, "Her2Raw.RData", sep="/"), compress="xz")
cat('Her2Raw.RData saved \n')
